...
async def on_message(message: DeliveredMessage):
    try:
        ...
        scheduled_time = datetime.fromisoformat(
            headers.get("scheduled_time") # type:ignore
        ...
        if current_time >= scheduled_time:
            ...
            await message.channel.basic_ack(
                delivery_tag=message.delivery.delivery_tag  # type:ignore
            )
        else:
            # Считаем время до обработки в миллисекундах
            delay = int((scheduled_time - current_time).total_seconds() * 1000)
            print(
                f"Deferring message: {message.body.decode()}, delay: {delay} ms"
            )
...
async def create_channel(connection):
    ...
    await channel.queue_bind(
        "main_queue", "delayed_exchange", routing_key="main_queue"
    )
